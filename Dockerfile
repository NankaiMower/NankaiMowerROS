...
COPY --link --from=fetch /opt/NankaiMowerROS/src/lib/slic3r_coverage_planner /opt/slic3r_coverage_planner_ws/src
...
COPY --link --from=fetch /opt/NankaiMowerROS /opt/NankaiMowerROS

#delete prebuilt libs (so that they won't shadow the preinstalled ones)
RUN rm -rf /opt/NankaiMowerROS/src/lib/slic3r_coverage_planner /apt-install_list

WORKDIR /opt/NankaiMowerROS

RUN bash -c "source /opt/ros/$ROS_DISTRO/setup.bash && cd /opt/NankaiMowerROS/src && catkin_init_workspace && cd .. && source /opt/prebuilt/slic3r_coverage_planner/setup.bash && catkin_make -DCATKIN_BLACKLIST_PACKAGES=slic3r_coverage_planner"

COPY .github/assets/NankaiMower_entrypoint.sh /NankaiMower_entrypoint.sh
